...
#
# Note:
# - Based on version from shell-scripts repo (and mezcla).
...
# Variables
## DEBUG: set -o xtrace
REPO_URL="https://github.com/tomasohara/search-diff-engine"
REPO_DIR_NAME="visual-diff"
...
if [ ! -e ~/.actrc ]; then
    echo "Error: run maldito act to configure default image in ~/.actrc"
    exit
fi
